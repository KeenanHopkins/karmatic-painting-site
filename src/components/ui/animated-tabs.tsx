...
const AnimatedTabs = ({
  ...
  return (
    <div className={cn("w-full flex flex-col gap-y-1 max-w-full", className)}>
      <div className="flex gap-2 flex-wrap bg-[#11111198] bg-opacity-50 backdrop-blur-sm p-1 rounded-xl w-full sm:w-fit mx-auto max-w-full justify-center">
        {tabs.map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="p-4 bg-[#11111198] shadow-[0_0_20px_rgba(0,0,0,0.2)] text-white bg-opacity-50 backdrop-blur-sm rounded-xl border min-h-60 h-full max-w-3xl w-full mx-auto overflow-hidden">
        {tabs.map(
          (tab) =>
            activeTab === tab.id && (
              <motion.div
